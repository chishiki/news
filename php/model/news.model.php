...
final class News extends ORM {
	...
	public function __construct($newsID = null) {
		...
		if ($newsID) {
			...
			$query = 'SELECT * FROM news_News WHERE ' . implode(' AND ', $whereClause) . ' LIMIT 1';
	...
	public function markAsDeleted() {
		...
		self::update($this, $conditions, true, false, 'news_');
...
final class NewsList {

	private $news;

	public function __construct(NewsListParameter $arg) {
		...
		$query = 'SELECT ' . $selector . ' FROM news_News WHERE ' . implode(' AND ',$where) . ' ORDER BY ' . implode(', ',$orderBy);
